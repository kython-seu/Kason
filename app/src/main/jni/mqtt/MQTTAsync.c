...
int MQTTAsync_addCommand(MQTTAsync_queuedCommand* command, int command_size)
{
	...
	LOGI("before MQTTAsync_unlock_mutex");
	MQTTAsync_unlock_mutex(mqttcommand_mutex);
	LOGI("after MQTTAsync_unlock_mutex");
#if !defined(WIN32) && !defined(WIN64)
	rc = Thread_signal_cond(send_cond);
	LOGI("RC IS %d",rc);
	if (rc != 0) {
		LOGE("Error %d from signal cond", rc);
...
{
	LOGI("going to destroy client handler");
	MQTTAsyncs* m = *handle;
	LOGI("MQTTAsyncs* m = *handle");
	FUNC_ENTRY;
	MQTTAsync_lock_mutex(mqttasync_mutex);
	...
	LOGI("has free responses and commands");
	if (m->c)
	{
	...
	*handle = NULL;
	LOGI("*handle = NULL free MQTTAsync *");
...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	MQTTAsync_freeServerURIs(m);
	LOGI("STRUCTVERSION is %d serverURIcount is %d",options->struct_version,options->serverURIcount);
	...
	if (options)
	{
		LOGI("OPTIONS ISN'T NULL it put callback function");
