...
{
	//LOGI("---%d",*((int *)rc));
	//LOGI("GO TO StackTrace_exit");
	//LOGI("going to exit");
	Thread_lock_mutex(stack_mutex);
	//LOGI("setStack %d",setStack(0));
	if (!setStack(0))
		goto exit;
	//LOGI("CURRENT depth");
	//LOGI("current_depth is %d",cur_thread->current_depth);
	if (--(cur_thread->current_depth) < 0) {
		//LOGI("Minimum stack depth exceeded for thread %lu", cur_thread->id);
	...
	if (strncmp(cur_thread->callstack[cur_thread->current_depth].name, name, sizeof(cur_thread->callstack[0].name)-1) != 0) {
		...
		LOGI("Stack mismatch. Entry:%s Exit:%s\n",
			 cur_thread->callstack[cur_thread->current_depth].name, name);
	}
	if (trace_level != -1)
	{
